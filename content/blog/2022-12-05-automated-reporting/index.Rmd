...
date: '2022-12-05'
slug: []
categories: ["Word", "PowerPoint", "Reporting"]
tags: []
csl: vancouver.csl
editor_options: 
  markdown: 
...
I'm happy to announce the initial release of `nlmixr2rpt`: a package designed to automate reporting of `nlmxir2` results to Word and PowerPoint. The nlmixr2rpt package is attempting to save you time on the back-end of your analysis so you can focus on the analysis itself. There are three main issues we are attempting to address:
...
* Reproducability: Manually creating Office documents with tweaks here and there is difficult to QC and difficult to replicate in a regulatory environment. 

# But how does it work?

Lets say you have an `nlmxir2` fit object like the one found in `nlmixr2extra`:   
...
There is a [vignette](https://nlmixr2.github.io/nlmixr2rpt/articles/Reporting_nlmixr_Fit_Results.html) that describes the format of that YAML file in detail but at a high level there are sections where figures and tables are defined. This is where you would create the code to say build out your observed vs predicted figure. Any packages you would normally use ([xpose.nlmxir2](https://cran.r-project.org/package=xpose.nlmixr2), [ggPMX](https://cran.r-project.org/package=ggPMX), etc.) can be used here.  You can construct these generically with placeholders that can be defined in the YAML file and overwritten when generating the report. 
